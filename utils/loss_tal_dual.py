...
class BboxLoss(nn.Module):
	...
	def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
		...
		iou = shape_iou(pred_bboxes_pos, target_bboxes_pos, xywh=False, scale=1.0)
		loss_iou = 1.0 - iou
...
class ComputeLoss:
	...
	def __call__(self, p, targets, img=None, epoch=0):
		...
		loss_mian = torch.zeros(3, device=self.device)  # box, cls, dfl
		...
		loss[1] *= 0.25
		loss1_ = self.BCEcls(pred_scores2, target_scores2.to(dtype)).sum() / target_scores_sum2  # BCE
		loss[1] += loss1_
		loss_mian[1] = loss1_
		...
		return loss.sum() * batch_size, loss.detach(), loss_mian.detach()  # loss(box, cls, dfl)
