...
class Semantic_Metrics:
	...
	def update(self, pred_masks, target_masks):
		...
		for b in range(nb):
			...
			output_masks = (torch.zeros(semantic_mask.shape).to(self.device)).scatter(1, max_idx.unsqueeze(1),
			                                                                          1.0)  # one hot: (h x w) x class
	...
	def results(self):
		...
		freq_ious = torch.zeros(1, dtype=torch.long).to(self.device) if (0 == total_c_bit_counts) else (
					                                                                                               self.c_bit_counts / total_c_bit_counts) * c_iou
