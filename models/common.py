...
class Conv2(Conv):
	# 主分支上使用的卷积层
	def forward(self, x):
		return self.act(self.bn(self.conv(x[0] if isinstance(x, list) else x)))


class Conv2a(Conv):
	# 辅助分支上使用的卷积层
	def forward(self, x):
		return self.act(self.bn(self.conv(x[1] if isinstance(x, list) else x)))
...
class MyCBFuse(nn.Module):
	def __init__(self, idx):
		super(MyCBFuse, self).__init__()
		self.idx = idx

	def forward(self, xs):
		target_size = xs[-1].shape[2:]
		res = [F.interpolate(x[self.idx[i]], size=target_size, mode='nearest') for i, x in enumerate(xs[:-1])]
		out = torch.sum(torch.stack(res + xs[-1:], dim=1), dim=1)
		return out


class ConvCBFuse(nn.Module):
	def __init__(self, c1, c2, idx):
		super(ConvCBFuse, self).__init__()
		self.idx = idx
	...
	def forward(self, xs):
		target_size = xs[-1].shape[2:]
		res = [F.interpolate(x[self.idx[i]], size=target_size, mode='nearest') for i, x in enumerate(xs[:-1])]
		out = torch.concat(res + xs[-1:], dim=1)
		out = self.conv(out)
		return out


class ImplicitConvCBFuse(nn.Module):
	def __init__(self, c1, c2, idx):
		super(ImplicitConvCBFuse, self).__init__()
		self.idx = idx
		self.conv = Conv(c1, c2, 3)
		self.ia = ImplicitA(c1)
		self.im = ImplicitM(c2)

	def forward(self, xs):
		target_size = xs[-1].shape[2:]
		res = [F.interpolate(x[self.idx[i]], size=target_size, mode='nearest') for i, x in enumerate(xs[:-1])]
		out = torch.concat(res + xs[-1:], dim=1)
		# out = (xs + self.ia) * self.im
		out = self.ia(out)
		out = self.conv(out)
		out = self.im(out)
		return out
