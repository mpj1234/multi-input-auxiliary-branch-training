...
from models.dbb_block import DiverseBranchBlock as DBB
...
class DBBDualDDetect(nn.Module):
	# YOLO Detect head for detection models
	dynamic = False  # force grid reconstruction
	export = False  # export mode
	shape = None
	...
	def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
		...
		self.cv3 = nn.ModuleList(
			nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch[:self.nl])
		self.cv4 = nn.ModuleList(
			nn.Sequential(DBB(x, c4, 3), DBB(c4, c4, 3, groups=4), nn.Conv2d(c4, 4 * self.reg_max, 1, groups=4)) for x in
			ch[self.nl:])
		self.cv5 = nn.ModuleList(
			nn.Sequential(DBB(x, c5, 3), DBB(c5, c5, 3), nn.Conv2d(c5, self.nc, 1)) for x in ch[self.nl:])
		self.dfl = DFL(self.reg_max)
		self.dfl2 = DFL(self.reg_max)
	...
	def forward(self, x):
		...
		if self.training:
			...
			if len(d1) > 0:
				...
				self.anchors, self.strides = (d2.transpose(0, 1) for d2 in make_anchors(d2, self.stride, 0.5))
			self.shape = shape
...
class DetectionModel(BaseModel):
	...
	def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
		...
		if ch == 6:
			# 双输入
			self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch // 2])  # model, savelist
		else:
		...
		if isinstance(m, (DualDetect, TripleDetect, DualDDetect, TripleDDetect, DBBDualDDetect)):
			s = 256  # 2x min stride
			m.inplace = self.inplace
			...
			if ch == 6:
				...
				m.stride = torch.tensor(
					[s / x.shape[-2] for x in forward([torch.zeros(1, 6 // 2, s, s), torch.zeros(1, 6 // 2, s, s)])])  # forward
			else:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
	...
	for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
		...
		if m in {
			Conv, AConv, ConvTranspose, Conv2, Conv2a,
			...
			c2 = ch[f[-1]]
		elif m in {ConvCBFuse, ImplicitConvCBFuse}:
			c2 = ch[f[-1]]
			_c1 = []
			for x in f:
				_c1.append(ch[x])
			c1 = []
			for _c1_i, c in enumerate(args[0]):
				c1.append(_c1[_c1_i][c])
			c1 = sum(c1 + _c1[-1:])
			args = [c1, c2, *args]
		# TODO: channel, gw, gd
		elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment, DBBDualDDetect}:
...
if __name__ == '__main__':
	...
	parser.add_argument('--cfg', type=str, default='yolov9-cn-double-ImplicitConvCBFuse-dbb.yaml', help='model.yaml')
	parser.add_argument('--ch', type=int, default=6, help='输入通道数目')
	...
	# Create model
	im = torch.rand(opt.batch_size, opt.ch, 640, 640).to(device)
	model = Model(opt.cfg, ch=opt.ch).to(device)
	model.eval()
