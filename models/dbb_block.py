...
class BNAndPadLayer(nn.Module):
	...
	# 这两个不使用，是因为如果用了，会在优化器参数分组时候导致重复把bn的训练参数分组
	# @property
	# def weight(self):
...
def transI_fusebn(kernel, bn):
	# 这两个不使用，是因为如果用了，会在优化器参数分组时候导致重复把bn的训练参数分组
	gamma = bn.bn.weight if isinstance(bn, BNAndPadLayer) else bn.weight
	bn_bias = bn.bn.bias if isinstance(bn, BNAndPadLayer) else bn.bias
	running_mean = bn.bn.running_mean if isinstance(bn, BNAndPadLayer) else bn.running_mean
	running_var = bn.bn.running_var if isinstance(bn, BNAndPadLayer) else bn.running_var
	eps = bn.bn.eps if isinstance(bn, BNAndPadLayer) else bn.eps

	std = (running_var + eps).sqrt()
	return kernel * ((gamma / std).reshape(-1, 1, 1, 1)), bn_bias - running_mean * gamma / std
...
if __name__ == '__main__':
	...
	# dbb = DiverseBranchBlock(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=3 // 2,
	...
	dbb = DiverseBranchBlock(in_channels=256, out_channels=512, kernel_size=3, groups=4)
	dbb.eval()
	train_y = dbb(x)
	print(train_y.shape)
	torch.save(dbb.half(), 'dbb.pth')
	dbb = torch.load('dbb.pth')
	dbb2 = DiverseBranchBlock(in_channels=256, out_channels=512, kernel_size=3, groups=4)
	dbb2.eval()
	for k, v in dbb.state_dict().items():
		dbb2.state_dict()[k] -= dbb2.state_dict()[k]
		dbb2.state_dict()[k] += v
	deploy_y2 = dbb2(x)
	print('========================== The diff is')
	print(((train_y - deploy_y2) ** 2).sum())
	dbb2.switch_to_deploy()
	deploy_y2 = dbb2(x)
	print('========================== The diff is')
	print(((train_y - deploy_y2) ** 2).sum())
