...
import val_dual_double_input as validate  # for end-of-epoch mAP
from models.experimental import attempt_load
from models.yolo import Model
from utils.autobatch import check_train_batch_size
from utils.callbacks import Callbacks
from utils.dataloaders import create_dataloader, create_dataloader_double_input
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
	...
	# Trainloader
	train_loader, dataset = create_dataloader_double_input(train_path,
	                                                       imgsz,
	...
	for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
		...
		optimizer.zero_grad()
		for i, (imgs, imgsd, targets, paths, _) in pbar:  # batch ------------------
			callbacks.run('on_train_batch_start')
			ni = i + nb * epoch  # number integrated batches (since train start)
			imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
			imgsd = imgsd.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
			imgs_arr = [imgs, imgsd]
			...
			with torch.cuda.amp.autocast(amp):
				pred = model(imgs_arr)  # forward
				loss, loss_items, _ = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
				if RANK != -1:
...
def parse_opt(known=False):
	...
	parser.add_argument('--cfg', type=str, default=ROOT / 'models/detect/yolov9-cn-double-ImplicitConvCBFuse-dbb.yaml',
	                    help='model.yaml path')
	parser.add_argument('--data', type=str, default=ROOT / 'data/wb.yaml', help='dataset.yaml path')
	parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-high-wb.yaml',
